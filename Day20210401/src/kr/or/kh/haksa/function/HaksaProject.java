...
public class HaksaProject {
	...
	public static void main(String[] args) {
		...
		if(cnt==1) {// 등록의 시작
			HaksaFunction.register(cnt, input, haksaList);  //학사 프로그램의 기능 클래스.등록 (register)
			 if(cnt==4) {
				continue;
			}
		}// 등록의 끝 !
		else if(cnt==2) { //찾기의 시작
			HaksaFunction.search(input, haksaList); //학사 프로그램의 기능 클래스 . 찾기 (search)
			...
			String commit = HaksaFunction.update(input, haksaList);
